...
class customData(Dataset):
    ...
    def __getitem__(self, item):
        ...
        affine_flag = random.uniform(0, 1)
        if affine_flag > 0.6 and self.phase == 'train':
            x_affine, y_affine, img = affine_img(img)
            pixel_label[0] += x_affine
            pixel_label[1] += y_affine
...
def affine_img(pil_image):
    ...
    h, w = img.shape[:2]
    x_affine = random.uniform(-0.3 * w, 0.3 * w)
    y_affine = random.uniform(-0.3 * h, 0.3 * h)
...
def main():
    note = '4_b4 epoch_1000 lr_1e-4 bsz_4 fine'
    ...
    img_root = './dataset/train_loc_fine/'
    txt_root = './dataset/train_loc_fine/'
    tr_img_path = img_root + 'img_{}/'.format(opt.target)
    tr_txt_path = txt_root + 'train_name_loc_{}.txt'.format(opt.target)
    te_img_path = img_root + 'img_{}/'.format(opt.target)
    ...
    pretrained_path = './result/2021.8.31/4_b4_6.31_epoch_1000_lr_0.0001_bsz_4_fine/best_model_856.pth'
    ...
    best_losses = [1000.0, 1000.0, 1000.0]
    ...
    for e in range(1, opt.epochs + 1):
        ...
        if e <= opt.epochs * 1 // 3:
            idx = 0
        elif opt.epochs * 1 // 3 < e <= opt.epochs * 2 // 3:
            idx = 1
        else:
            idx = 2
        if pixel_eval_loss < best_losses[idx]:
            best_losses[idx] = pixel_eval_loss
            best_models[idx] = copy.deepcopy(model.state_dict())
            best_epochs[idx] = e
    ...
    for best_loss in best_losses:
        best_losses_str += '%.2f' % best_loss + '_'
    best_losses_str = best_losses_str.strip('_')

    # save statistics
    data_frame = pd.DataFrame(data=results, index=range(1, opt.epochs + 1))
    # print(data_frame)
    data_frame.to_csv('{}/best_loss_{}.csv'.format(opt.result_path, best_losses_str),
                      index_label='epoch')

    # save model
    for i in range(1, len(best_models)):
        path = opt.result_path + 'best_model_{}.pth'.format(best_epochs[i])
        torch.save(best_models[i], path)
    path = opt.result_path + 'last_model.pth'
    torch.save(last_model, path)
    print('Best Epoch:{}, Best Loss: {}, '.
          format(best_epochs, best_losses))
    print('note:', note)
    print(opt)
